---
title: "Resources — Complete Reference"
description: "The Resource class is the core building block of Martis, and each resource maps to the Eloquent model you administer through it. It defines the model's CRUD interface, fields, authorization, and behavior."
sourcePath: "martis-package/docs/resources.md"
---
...
Pin the namespace explicitly (`'resources_namespace' => 'App\\Martis\\Customer'`) when the directory is not autoloaded. See [Configuration → Resources path and namespace](/docs/reference/configuration#resources-path-and-namespace).
...
See the [Fields Reference](/docs/core/fields) for all 50 available field types.
...
See [Filters](/docs/core/filters), [Lenses](/docs/core/lenses), [Metrics](/docs/core/metrics), and [Dashboards](/docs/core/dashboards) for the per-class authoring guide.
...
If `menuCount()` throws, the badge is hidden for that user (the rest of the sidebar renders) and the failure is reported through `report()` as a `Martis\Exceptions\MenuCountFailedException` naming the resource class (v1.32.4). See [Menus → When a badge is missing](/docs/core/menus#when-a-badge-is-missing) for the dev-mode `_failed` diagnostics and the `counts.report_failures` switch.
...
Tools have the same opt-in since **v1.35.0**: `Tool::withSystemSection()` docks a Tool in the same section, after the resources and before the Cache admin link. See [Tools → Place a Tool under "System"](/docs/customization/tools#place-a-tool-under-system--withsystemsection-v1350).

Since **v1.38.0** the section is ordered by a weight. Override `systemSectionOrder(): int` (default `100`) to move the resource: a lower weight lists it earlier, and entries with equal weights keep the natural order (resources in registration order, then Tools, then the cache link at `1000`). See [Menus → Order inside the System section](/docs/core/menus#order-inside-the-system-section-v1380).
...
Controls whether this resource participates in the [Sticky Views](/docs/core/sticky-views) feature — per-user view state persistence on the index page (filters, sort, pagination, per-page, search). Default: `true`.
...
Lenses use the same shape (see [lenses.md](/docs/core/lenses)) — they inherit nothing from the resource, so each Lens declares its own polling cadence if it wants one.
...
3. Relation fields (`HasMany`, `MorphMany`, `BelongsToMany`, `MorphToMany`) inherit the related resource's `perPageOptions()` when the developer did not call `->perPageOptions([...])` on the field itself. See [relationships.md](/docs/core/relationships).
...
| `Lens::resolvedPerPage(): int` | Same rule for lenses. | See [lenses.md — Effective per-page](/docs/core/lenses#effective-per-page). |
...
Constrains the index listing query and the queries Martis builds like it: the count badge, the global search (its results and `total`), the records an action runs on, and the parent record of a `BelongsToMany` panel and of the pivot routes. The canonical place for multi-tenancy, ownership scoping, or any other structural filter. The declarative [`scopes()`](/docs/auth/authorization#declarative-query-scopes) run first wherever it runs. A lens owns its query, as in Nova, so repeat such a filter in the lens's `query()`; the relationship pickers use `relatableQuery()` below.
...
Constrains the query used to list candidate records in every relationship picker that targets this resource: BelongsTo dropdowns, the context-free relatable form, and the BelongsToMany / MorphToMany attach pickers. It is the resource's own fence and always applies; a source resource's `relatable{PluralModelName}()` and a field's `relatableQueryUsing()` narrow on top of it, never replace it (see [Relationships → Relatable scoping precedence](/docs/core/relationships#relatable-scoping-precedence)). A resource that confines its index with `indexQuery()` or `scopes()` on a model that cannot carry a global scope should declare the same predicate here so the fence holds on the pickers too: neither hook applies to a picker.

The writes follow it too, as Nova's `Relatable` rule: a create, update, attach or Action run that names a record outside the query answers 422 on the field (see [Relationships → Writes follow the pickers](/docs/core/relationships#writes-follow-the-pickers)). So keep it as wide as what the app actually saves.
...
See [Default Row Actions — Row-click redundancy](/docs/core/default-row-actions#row-click-redundancy) for the full decision matrix.
...
On the full-page routes the browser back button is held only while the form has unsaved changes (v1.38.0+); a clean form leaves Back and Forward working as on any other page. Before v1.38.0 opening a create or edit page erased the Forward history, and Back could skip the previous page. See [Overrides → Unsaved changes guard](/docs/customization/overrides#unsaved-changes-guard).
...
See the [Fields Reference](/docs/core/fields) for all 50 field types and validation methods.
...
- **Reset view** — the bigger hammer that clears filters + search + sort + per-page + trashed-toggle and forgets the [sticky view](/docs/core/sticky-views) for this resource. Visible whenever any of those drift away from the resource's defaults.
...
See the [Override System](/docs/customization/overrides) documentation for full details.
