...
By default the resolver matches with `ILIKE '%term%'` over `searchable()` fields (plus `searchableRelations()`). Override `searchQuery()` to **replace the match predicate entirely** — for driver-native search (PostgreSQL full-text `@@`, `pg_trgm` fuzzy, MySQL `FULLTEXT`, SQLite FTS5) — without Scout and without leaving the resource pipeline. Return a `Builder` and the resolver applies **none** of its own matching (no `ILIKE`, no `field:value` parsing, no priority ranking, and it skips the empty-set `1 = 0` guard); return `null` (the default) to keep the built-in `ILIKE`. Filters, the user's sort, and pagination are untouched, and it applies to the resource index, global Cmd+K, and relation pickers alike.
...
:::caution[You own the WHERE — in place]
