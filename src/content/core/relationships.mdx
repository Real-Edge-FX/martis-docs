...
A value the request sends for a skipped field still runs the field's rules. A pivot update with nothing left to write (an empty body, or only readonly and immutable values) answers 200 and leaves the row as it was. See [Fields → Immutable fields](/docs/core/fields#immutable-fields).
...
See [fields.md § MorphToMany](/docs/core/fields#morphtomany) for the full API.
...
The [Slug](/docs/core/fields#slug) collision check reads the forms in the same order (the update form when its `id` names a record). The [`dependsOn` sync](/docs/core/fields#reactive-fields--dependsonfield-closure) and the server-side [`Select` search](/docs/core/fields#select) take the form from their `context` parameter and search only that form (`create` includes `fieldsForInlineCreate()`), never `fields()`, so a field that is not on the form cannot be probed.
