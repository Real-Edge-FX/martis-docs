...
Pivot values are written as the request sends them: a pivot field does not honour `immutable()`, so the pivot update overwrites it. See [Fields → Immutable fields](/docs/core/fields#immutable-fields).
