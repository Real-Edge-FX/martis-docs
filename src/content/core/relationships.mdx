---
title: "Relationships"
description: "Martis has a field for every Eloquent relationship, from BelongsTo and HasMany to the polymorphic MorphTo and MorphToMany. This guide covers how each one is declared, how it renders (a picker, a card or an inline panel) and how it is authorized."
sourcePath: "martis-package/docs/relationships.md"
---
...
`viewAny` (see
[Authorization → `viewAny` is the entry gate](/docs/auth/authorization#viewany-is-the-entry-gate)).
...
rule (see [Actions → The Action Events panel](/docs/core/actions#the-action-events-panel-v201)).
...
5. **Actions column** — View / Edit / Delete, plus Restore and Force-delete
   ...
   [Actions → In relationship panels](/docs/core/actions#in-relationship-panels)).
...
| Inside a custom override, Tool or card | The record it names with `NestedParentProvider` from `@martis/runtime` (`id: null` on a create form), else the record in the URL. See [overrides.md § Naming the record of the relationship panels](/docs/customization/overrides#naming-the-record-of-the-relationship-panels-v1380). |
...
Declarative query scopes](/docs/auth/authorization#declarative-query-scopes).
...
[Resources → indexQuery()](/docs/core/resources#indexquery).
...
See [fields.md — BelongsTo](/docs/core/fields#belongsto) for full API reference.
...
See [fields.md — HasMany](/docs/core/fields) for full API reference.
...
Pivot fields validate with their own rules, like any field: the attach runs `rules()` plus `creationRules()`, and the pivot update runs `rules()` plus `updateRules()` with the literal `required` dropped and `sometimes` first, so a pivot field the update does not send is left alone. Rule objects (`Rule::in()`, `Rule::unique()`), `ValidationRule` instances and closures run on both. See [Fields → What an update validates](/docs/core/fields#what-an-update-validates).
...
- a pivot `Repeater` writes its rows as on a record: a row keeps the stored value of a row field it cannot write and a new row takes the field's `default()`, and its rows are sent back as the Repeater reads them, without the row fields the user cannot see (v1.38.0+, see [Repeater → Readonly, computed, hidden and immutable row fields](/docs/core/repeater#readonly-computed-hidden-and-immutable-row-fields)).
...
Actions declared on the field with `->actions()` run on the rows selected in this relationship's panel, and `handle()` receives each related model with its `pivot` row (the pivot fields above included). A resource action flagged `->pivotAction()` shows on every `BelongsToMany` and `MorphToMany` panel of the resource. See [Actions → Pivot Actions](/docs/core/actions#pivot-actions).
...
See [fields.md — MorphTo](/docs/core/fields#morphto) for full API reference.
...
See [fields.md § HasOne](/docs/core/fields#hasone) for the full API.
...
See [fields.md § HasOneOfMany](/docs/core/fields#hasoneofmany) for the full API.
...
See [fields.md § HasOneThrough](/docs/core/fields#hasonethrough) for the full API.
...
See [fields.md § MorphOne](/docs/core/fields#morphone) for the full API.
...
See [fields.md § HasManyThrough](/docs/core/fields#hasmanythrough) for the full API.
...
See [fields.md § MorphToMany](/docs/core/fields#morphtomany) for the full API. The relation pickers among its pivot fields ask the panel, under `.../morph-to-many/{relationship}/pivot-fields/...`, like a `BelongsToMany`'s (see [Relation pickers among the pivot fields](#relation-pickers-among-the-pivot-fields)).
...
The [Slug](/docs/core/fields#slug) collision check reads the forms in the same order (the update form when its `id` names a record the user may update, and only that record is left out of the uniqueness probe). The [`dependsOn` sync](/docs/core/fields#reactive-fields--dependsonfield-closure) and the server-side [`Select` search](/docs/core/fields#select) take the form from their `context` parameter and search only that form (`create` includes `fieldsForInlineCreate()`), never `fields()`, so a field that is not on the form cannot be probed.
