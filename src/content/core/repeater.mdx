---
title: "Repeater"
description: "Repeatable row widget backed by JSON, a child table (HasMany), or a single polymorphic child table. Ships five differentials: parent-context injection (dependsOn), collapsible rows with cardinality limits, dynamic row headers, row templates with duplicate and bulk paste, and a po"
sourcePath: "martis-package/docs/repeater.md"
---
...
- **In the form.** Each error shows under the field of the row it belongs to
  ...
  [Overrides](/docs/customization/overrides)).
...
field (see [Fields → Immutable fields](/docs/core/fields#immutable-fields)).
