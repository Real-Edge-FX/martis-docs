---
title: "Filters"
description: "Filters allow users to narrow down resource index results by applying criteria. They appear as a collapsible panel on the resource index page with active filter pills for at-a-glance visibility."
sourcePath: "martis-package/docs/filters.md"
---
...
**Options format:** keys are display labels, values are passed to `apply()`. This is Nova's filter order, the reverse of a field's `options()`, which reads `[value => label]` as Nova's fields do (see [Fields → Option order](/docs/core/fields#option-order)).
