---
title: "Metrics"
description: "Metrics compute and display analytical data on dashboards and resource index pages. Martis provides six built-in metric types: Value, Trend, Partition, Progress, ActivityFeed, and EndpointTable."
sourcePath: "martis-package/docs/metrics.md"
---
...
`widthMd()` is the explicit form of the `md` tier; use it when the card should change again at `lg` (`width(6)->widthMd(12)->widthLg(4)` reads as tablet 12, desktop 4). A plain custom `Card` (see [Dashboards → Custom cards](/docs/core/dashboards#custom-cards-filter-reactive)) accepts the same three methods.
...
Metric results are cached by default through the Martis `metrics` cache layer (5 minutes, see [Cache](/docs/reference/cache)). Override `cacheFor()` to give one metric its own lifetime instead:
