---
title: "Fields — Complete Reference"
description: "All available field types in Martis, their methods, and configuration options."
sourcePath: "martis-package/docs/fields.md"
---
...
| `fill` | `fill(Model $model, mixed $value): void` | Write a value to the model. Respects the `fillUsing()` callback, the `readonly` flag and `computed()` (a computed field writes nothing unless a `fillUsing()` callback translates it), on every built-in field (`Repeater` honours both since v1.38.0, and applies them, `canSee()` and `immutable()` to the fields inside its rows, see [Repeater](/docs/core/repeater#readonly-computed-hidden-and-immutable-row-fields)). |
...
- **Action fields.** An Action field the user cannot see is left out of the modal, is not validated and never takes its value from the request. See [Actions → Fields the request cannot set](/docs/core/actions#fields-the-request-cannot-set).
- **Sorting.** A list is sorted only by a `sortable()` field the user can see (v1.38.0): the order of the rows would otherwise tell the order of the field's values. A `?sort=` naming a field `canSee()` hides is ignored like one naming an unknown attribute, and the list keeps its default order. This holds on every list that takes `?sort=`: the resource index (its `defaultSort()` too), the rows of a `HasMany`, `MorphMany`, `BelongsToMany` and `MorphToMany` panel, and a lens, which is sorted by its own sortable fields only (see [Lenses → Which columns sort a lens](/docs/core/lenses#which-columns-sort-a-lens)).
- **Searching.** A search matches its term only on the `searchable()` fields the user can see (v1.38.0): the rows a term returns would otherwise tell which records hold it in a field the user may not read. This holds on every search built from the fields' `searchable()` flags: the resource index, the rows of a relationship panel and its attach picker, the global search (see [Global Search → Fields the user cannot see](/docs/core/global-search#fields-the-user-cannot-see)) and a relation picker (`BelongsTo`, `MorphTo`, `Tag`), which searches the title it shows when none of the related resource's searchable fields is left. A `field:value` token naming a field the user cannot see is dropped like one naming an unknown field. What a resource declares itself is searched as declared: its `searchableRelations()` paths, its `searchQuery()` predicate and, with Scout, its `toSearchableArray()`.
- **Layouts.** The rules are the same whatever layout container holds the field: `Panel`, `Section`, or directly a `Tab`.

A field inside a Repeater row follows `canSee()` (see [Repeater → Readonly, computed, hidden and immutable row fields](/docs/core/repeater#readonly-computed-hidden-and-immutable-row-fields)); `canSeeForModel()` is not applied to row fields.
...
The forms follow the same split. The update forms render an immutable field read-only, exactly as they render a `readonly()` field, and still submit its stored value: the update page (also when a relation panel opens the record to edit it), the update drawer (`DrawerOverride::update()`) and the form that edits a pivot row. The create page, the create drawer, the inline create and the attach keep the input editable. The schema serialises the flag as `immutable`; a form built with `useMartisForm({ context: 'update' })` (see [Tool fields](/docs/customization/tool-fields)) resolves it the same way, and a custom input registered for the field type receives the field with `readonly: true`. A field inside a `Repeater` row is immutable row by row (v1.38.0+): a new row writes and validates the value it sends, a row the record stores keeps its value, on every storage mode, and an update form renders the field read-only on the stored rows only; an immutable `Repeater` as a whole is skipped on update like any field. See [Repeater → Readonly, computed, hidden and immutable row fields](/docs/core/repeater#readonly-computed-hidden-and-immutable-row-fields). Up to v1.37.3 every form rendered an immutable field as an editable input, and the update dropped the new value silently.
...
Controls how the field renders as a column in the index table. See [resources.md](/docs/core/resources#tablelayout) for the Resource-level `tableLayout()` switch.
...
All defaults can be disabled globally via `config('martis.index.column_defaults', false)` (env `MARTIS_INDEX_COLUMN_DEFAULTS`). Explicit per-field calls still apply. See [resources.md](/docs/core/resources#opting-out-of-the-type-default-heuristics) for details.
...
way, see [Tooltip Standard](/docs/customization/components#tooltip-standard-primereact)).
...
- Initials and palette shared with `UiAvatar`, the Topbar and the profile page through `Martis\Support\Initials` (the fields reach it via the [`ResolvesInitialsPayload`](https://github.com/Real-Edge-FX/martis-package/blob/main/src/Fields/Concerns/ResolvesInitialsPayload.php) trait).
...
> **Version note.** Martis v2.0 and later read `options()` as `[value => label]`, Nova's order; Martis v1.x read `[label => value]`. On v1.x, flip the arrays in the examples below, or upgrade: see [Upgrading](/docs/getting-started/upgrading).
...
- **Filters are the exception, as in Nova:** a filter's `options(Request $request)` returns `[label => value]`. See [Filters](/docs/core/filters).
...
— see [relationships.md § Toolbar hide flags](/docs/core/relationships#toolbar-hide-flags-cross-cutting).
...
| `actions` | `actions(Closure(Request): list<Action>): static` | `$this` | Pivot actions for this relationship's panel only; `handle()` receives the selected attached records, each with its `pivot` row. See [Actions → Pivot Actions](/docs/core/actions#pivot-actions). | `null` |
...
A relation picker among the pivot fields asks the panel (the two `pivot-fields` routes), gated like the panel and then like the attach (`attachAny{Model}`) or the pivot update (`updatePivot{Model}`). See [Relationships → Relation pickers among the pivot fields](/docs/core/relationships#relation-pickers-among-the-pivot-fields).
...
Without an override they ask the parent's policy (`attach{Model}`, `detach{Model}`; permitted when the policy does not define them). `attachAny{Model}` (`authorizedToAttachAny()`) gates the attach as a whole: when it denies, the list of records to attach, the attach and the attach modal's pivot pickers answer 403 (v1.38.0+). See [Relationships → Authorization](/docs/core/relationships#authorization).
...
See [relationships.md — HasOne](/docs/core/relationships#hasone) for the full guide.
...
All nine `hideXxx()` setters from `ControlsRelationshipToolbar` are inherited
(via `HasOne`) — see [relationships.md § Toolbar hide flags](/docs/core/relationships#toolbar-hide-flags-cross-cutting).
...
See [relationships.md — HasOneOfMany](/docs/core/relationships#hasoneofmany) for the
full guide.
...
[relationships.md § HasOneThrough](/docs/core/relationships#hasonethrough)).
...
See [relationships.md — HasOneThrough](/docs/core/relationships#hasonethrough) for
the full guide.
...
See [relationships.md — HasMany](/docs/core/relationships#hasmany) for the full
guide and [Relationship panel anatomy](/docs/core/relationships#relationship-panel-anatomy)
...
[relationships.md § HasManyThrough](/docs/core/relationships#hasmanythrough)).
...
All nine `hideXxx()` setters from `ControlsRelationshipToolbar` are inherited
(via `HasMany`) — see [relationships.md § Toolbar hide flags](/docs/core/relationships#toolbar-hide-flags-cross-cutting).
...
See [relationships.md — HasManyThrough](/docs/core/relationships#hasmanythrough) for
the full guide.
...
See [relationships.md — MorphOne](/docs/core/relationships#morphone) for the full
guide.
...
All nine `hideXxx()` setters from `ControlsRelationshipToolbar` are inherited
(via `MorphOne`) — see [relationships.md § Toolbar hide flags](/docs/core/relationships#toolbar-hide-flags-cross-cutting).
...
See [relationships.md — MorphOneOfMany](/docs/core/relationships#morphoneofmany) for
the full guide.
...
See [relationships.md — MorphMany](/docs/core/relationships#morphmany) for the full
guide and [Relationship panel anatomy](/docs/core/relationships#relationship-panel-anatomy)
for the shared shell layout.
...
All nine `hideXxx()` setters from `ControlsRelationshipToolbar` are inherited — see [relationships.md § Toolbar hide flags](/docs/core/relationships#toolbar-hide-flags-cross-cutting). On `MorphTo` only the *Create* button (inline create) and the *View* action surface are affected (per the trait scope matrix). Remember: visible = authorized AND NOT hidden. Authorization is always the source of truth; the hide flags can only hide, never force-visible.
*src/Fields/Concerns/ControlsRelationshipToolbar.php*
...
| `actions` | `actions(Closure(Request): list<Action>): static` | `$this` | Pivot actions for this relationship's panel only; `handle()` receives the selected attached records, each with its `pivot` row. See [Actions → Pivot Actions](/docs/core/actions#pivot-actions). | — |
| `searchable` | `searchable(bool $value = true): static` | `$this` | Enable search in the attach modal. | `false` |
| `collapsable` | `collapsable(bool $value = true): static` | `$this` | Make the panel collapsable. | `false` |
| `collapsedByDefault` | `collapsedByDefault(bool $value = true): static` | `$this` | Start collapsed. Implies `collapsable`. | `false` |
| `allowDuplicateRelations` | `allowDuplicateRelations(bool $value = true): static` | `$this` | Allow attaching the same record multiple times. | `false` |
...
See [relationships.md — MorphToMany](/docs/core/relationships#morphtomany) for the
full guide and [Relationship panel anatomy](/docs/core/relationships#relationship-panel-anatomy)
for the shared shell layout.
...
with a `type` discriminator). See the dedicated [Repeater guide](/docs/core/repeater) for the
full API. Highlights:
...
[Repeater → Relation pickers and remote selects in rows](/docs/core/repeater#relation-pickers-and-remote-selects-in-rows).
...
v1.38.0): see [Repeater § Validation](/docs/core/repeater#validation).
...
See [Override System](/docs/customization/overrides) for details.
...
2. Frontend navigates to `/resources/{resource}/create?fromResourceId={id}` (a resource with a create override opens that override instead, with `fromResourceId`; see [Overrides → Override Props](/docs/customization/overrides#override-props))
