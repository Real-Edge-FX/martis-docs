---
title: "Default Row Actions"
description: "Every Martis resource index ships with a trailing column of built-in row actions — **View**, **Edit**, and **Delete** — out of the box. No registration required. Icons disable themselves automatically when the row's authorization denies the operation."
sourcePath: "martis-package/docs/default_row_actions.md"
---
...
For destructive UI styling (red border, destructive button label) extend `Martis\Actions\DestructiveAction` instead of toggling a flag — see [actions.md § Destructive Actions](/docs/core/actions#destructive-actions).
