---
title: "Lenses"
description: "Lenses are alternative, resource-scoped views onto the dataset of a Martis Resource. Every lens is a subclass that composes its own query, fields, filters, cards, actions and optional polling cadence. Martis adds a small set of extensions that are marked below."
sourcePath: "martis-package/docs/lenses.md"
---
...
the clamped result. See [resources.md — Effective per-page](/docs/core/resources#effective-per-page)
...
[Fields → Field authorization](/docs/core/fields#field-authorization-cansee-and-canseeformodel)).
...
[API → Actions](/docs/reference/api#actions)), so an action the resource does
...
See [authorization.md](/docs/auth/authorization) for the broader policy
contract.
