---
title: "Actions"
description: "Martis Actions allow you to run custom operations on resources. Four distinct action types are supported:"
sourcePath: "martis-package/docs/actions.md"
---
...
The rows of a `Repeater` among the fields follow the Repeater's row rules, every row being a new one (an Action stores nothing): a readonly or hidden row field holds its `default()` or nothing, a computed one nothing, and an immutable one the value the row sends. See [Repeater → Readonly, computed, hidden and immutable row fields](/docs/core/repeater#readonly-computed-hidden-and-immutable-row-fields).
...
A picker in a row of a `Repeater` among the Action's fields adds the row to either request (`?repeater={attribute}&repeatable={type}`), and the field is read from that row type's `fields()` (v1.38.0+, see [Repeater → Relation pickers and remote selects in rows](/docs/core/repeater#relation-pickers-and-remote-selects-in-rows)).
...
The denied gate check runs with every navigation build, so the [authorization-denial audit](/docs/auth/authorization#audit-log-of-denied-authorizations) skips it like the `viewAny` cascade (unless `MARTIS_AUDIT_AUTHZ_DENIALS_INCLUDE_VIEWANY=true`).
