---
title: "Highlights"
description: "A tour of the surfaces Martis ships beyond the bare-essential admin-panel contract, each a first-class API of the package. They come with their own configuration, generators, or escape hatches, and each entry below links into the dedicated reference for the full surface."
sourcePath: "martis-package/docs/differentials.md"
---
...
Full guide: [Override System](/docs/customization/overrides).
...
[Tools → Tool routes and their middleware](/docs/customization/tools#tool-routes-and-their-middleware).
...
[Fields → Tooltips](/docs/core/fields#tooltips-martis-differential) for the full rationale and the
...
[Fields Reference](/docs/core/fields#icon).
...
[`ResolvesInitialsPayload`](https://github.com/Real-Edge-FX/martis-package/blob/main/src/Fields/Concerns/ResolvesInitialsPayload.php)
...
per-field inline styles. See [Fields Reference](/docs/core/fields#stack--line).
...
Edit and Delete. See
[relationships.md § Toolbar hide flags](/docs/core/relationships#toolbar-hide-flags-cross-cutting).
...
See [Grid Layout → Responsiveness](/docs/core/grid-layout#responsiveness).
...
Full documentation in [repeater.md](/docs/core/repeater).
...
with one rule. See [Metrics → Responsive widths](/docs/core/metrics#responsive-widths-widthmd--widthlg).
...
[metrics.md](/docs/core/metrics#activity-feed-metric-martis-extension).
...
[metrics.md](/docs/core/metrics#endpoint-table-metric-martis-extension).
...
Full reference: [sso.md](/docs/auth/sso).
...
[components.md](/docs/customization/components#event-bus)).
...
density / language / accessibility. See [preferences.md](/docs/reference/preferences)
...
See [Theming Guide](/docs/customization/theming) for the complete variable reference.
...
`martis:card` writes both the PHP class (`app/Martis/Cards/`) and the React component (`resources/js/martis-extensions/cards/{Name}.tsx`); the bundle's filename → key auto-discovery (`{Name}.tsx` → `card:{kebab-name}`) registers it on the next `npm run build:extensions`. `martis:override` writes a TSX-only file under `resources/js/martis-extensions/overrides/` and the bundle auto-registers it on the next build. See [Override System: Auto-registration scope](/docs/customization/overrides#6-creating-custom-components-artisan) for the full filename → key table.
...
| `martis:list-env-vars` | Markdown / JSON table of every `MARTIS_*` env var (used to refresh the env-var index in [configuration.md](/docs/reference/configuration)) |
