---
title: "Martis Differentials"
description: "Replace any React component at the right granularity level without forking the package."
...
See [Override System](/docs/customization/overrides) for the full guide.
...
a field type of their own. See [Fields → Tooltips](/docs/core/fields#tooltips-martis-differential)
...
Supports palette whitelisting (restrict to a configured subset), `colorFrom()` to pull the hex colour from another attribute, configurable sizes and tooltips. Full API lives in [Fields Reference](/docs/core/fields#icon).
...
Line variants — `asHeading()`, `asBase()`, `asSmall()`, `asMuted()`, `asCode()` — map to `.martis-line-*` classes so custom themes restyle every Line in the package through a handful of CSS tokens instead of per-field inline styles. See [Fields Reference](/docs/core/fields#stack--line).
...
API. Full documentation in [repeater.md](/docs/core/repeater).
...
Generator: `php artisan martis:activity-feed`. Full reference in [docs/metrics.md](/docs/core/metrics#activity-feed-metric-martis-extension).
...
Generator: `php artisan martis:endpoint-table`. Full reference in [docs/metrics.md](/docs/core/metrics#endpoint-table-metric-martis-extension).
...
The preferences panel is a compact topbar overlay — theme / accent / density / language / accessibility. See [preferences.md](/docs/reference/preferences) for the resolver, API, and SSR no-flash mechanics.
