...
`martis:card` writes both the PHP class (`app/Martis/Cards/`) and the React component (`resources/js/martis-extensions/cards/{Name}.tsx`); the bundle's filename → key auto-discovery (`{Name}.tsx` → `card:{kebab-name}`) registers it on the next `npm run build:extensions`. `martis:override` writes a TSX-only file under `resources/js/martis-extensions/overrides/`. See [Override System: Auto-registration scope](/customization/overrides#6-creating-custom-components-artisan) for which `--type` values auto-register and which require a manual `OVERRIDE_KEYS` extension.
