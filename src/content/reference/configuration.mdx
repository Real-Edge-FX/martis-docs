---
title: "Configuration"
description: "Martis is configured through config/martis.php, which the installer publishes; most of its options also read an environment variable. Publish it with the bundled wrapper (or the standard vendor:publish if you prefer):"
sourcePath: "martis-package/docs/configuration.md"
---
...
|-----|------|---------|-------------|
| `guard` | `?string` | `null` | Authentication guard. `null` uses Laravel's default guard. The panel's requests run as that guard (it becomes the request's guard), and the auth flows, the Martis migrations and `martis:user` use its provider: see [Upgrading → A custom Martis guard](/docs/getting-started/upgrading#a-custom-martis-guard). |
...
For **client-side navigation** inside the SPA (react-router), each page uses the [`usePageTitle`](/docs/customization/overrides#page-title-hook) hook to keep `document.title` in sync without a full reload.
...
|--------|-----------------|-----------------|
| `sidebar` (default) | Left nav column, topbar, content, in-flow footer. The collapsible shell described in [theming.md](/docs/customization/theming) and [overrides.md](/docs/customization/overrides). | Most admin apps. Handles mobile drawer, collapsed rail, accent pill, density tokens. |
...
Full wiring examples, prop contracts, and the rationale for piece-by-piece vs full-shell overrides live in [overrides.md](/docs/customization/overrides#shell-piece-by-piece-overrides).
...
See [menus.md](/docs/core/menus#count-badges) for the badge API (including `menuCount()` for custom values) and [menus.md](/docs/core/menus#sections) for the section heading API.
...
Martis UI strings come from the package's `resources/lang/{locale}/` files, overridden by the published copies in `lang/vendor/martis/{locale}/` (see [i18n](/docs/customization/i18n)). Publish translations with:
...
Custom themes are scaffolded via `php artisan martis:theme`, which writes the source and sets `name`. See [Theming → Theme files](/docs/customization/theming#theme-files).
...
Both flags are read live, so mid-session toggles via `window.MartisConfig` take effect on the next registration. See [Keyboard Shortcuts](/docs/reference/keyboard-shortcuts) for the full API.
...
Each item accepts `label` (i18n key or literal), `icon` (**Phosphor icon name** — as of v1.29.0, not a PrimeIcons class), `url` (internal path or external URL), and an optional `position` (`'before'` / `'after'` the built-in Profile entry). Full reference in [Authentication → User Menu](/docs/auth/authentication#user-menu).
...
See [Default Row Actions](/docs/core/default-row-actions) for the full guide.
...
Override per-resource via `perPage()` and `perPageOptions()` on the resource class. When the returned `perPage()` is not present in `perPageOptions()`, Martis silently clamps to `perPageOptions()[0]` — see [resources.md — Effective per-page](/docs/core/resources#effective-per-page).
...
The registered resource is closed until you define the `view-martis-action-events` gate (or an `ActionEvent` policy), v2.0.1+: see [Actions → Who can read the audit log](/docs/core/actions#who-can-read-the-audit-log-v201).
...
| `gates.plan_resolver` | `?Authenticatable $user` | The user's plan (`?string`). See [Soft-gates](/docs/core/gates#plan-rank-shortcut--for-linear-tier-saas). |
| `profile.avatar.url_resolver` | `string $storedPath` | The avatar's public URL (`string`). |
| `auth.sso.providers.{provider}.role_source_callable` | `string $externalId, string $accessToken` | The external role names (`array<int, string>`). See [SSO](/docs/auth/sso#role_source--where-external-role-names-come-from). |
| `auth.sso.providers.{provider}.role_callable` | `array $externalRoles, ?User $user, string $provider` | The local roles (`Collection`). See [SSO](/docs/auth/sso#role_strategy--how-to-map-external-names-to-local-roles). |
...
`admin_ui_order` (v1.38.0+) positions the "System cache" link inside the bundled System sidebar section, which sorts its entries by weight (System-section resources and Tools default to `100`, so `1000` keeps the link last). See [Menus → Order inside the System section](/docs/core/menus#order-inside-the-system-section-v1380).

Per-subsystem cache layer with three control planes (config / env / runtime), bypass header, and admin page. See [Cache](/docs/reference/cache) for the full reference.
...
See [Authentication](/docs/auth/authentication#user-profile) for full profile documentation.
...
See [User Preferences](/docs/reference/preferences) for the D1/D2/D3 spec and [i18n](/docs/customization/i18n) for the locale layer.
...
See [Sticky Views](/docs/core/sticky-views) for the full behaviour spec.
...
See [Loader](/docs/customization/loader) for the surface-by-surface behaviour matrix.
...
The package ships **no default Gate**. Define `martis-impersonate` in your `AuthServiceProvider` to gate which users can start an impersonation session. See [Impersonation](/docs/auth/impersonation).
...
Off by default. When enabled, mounts the Scramble-generated OpenAPI 3.1 surface at `/{path}` (default `/api-docs`). See [API → Overview](/docs/reference/api) for the worker-restart caveat: PHP-FPM caches parsed env in process memory, so flipping the env requires restarting workers (or the PHP container) before the gate sees the new value.
...
| `MARTIS_CACHE_SCHEMA` / `MARTIS_CACHE_SCHEMA_TTL` | `1440` | Schema TTL in minutes (one day since v2.0; `null` before, see [Cache → Invalidation](/docs/reference/cache#invalidation)) |
...
- [Installation Guide](/docs/getting-started/installation) — Initial setup
- [Authentication](/docs/auth/authentication) — Login, 2FA, profile
- [Resources](/docs/core/resources) — Resource configuration
- [Override System](/docs/customization/overrides) — Customize the UI
