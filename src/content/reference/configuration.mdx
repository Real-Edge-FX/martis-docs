...
- **Logo-only mode** — set `brand.logo` when you have a finished horizontal lockup (mark + wordmark in one asset). The lockup renders alone; the brand-name text Martis would normally print next to the mark is **hidden** so the wordmark is not duplicated.
- **Icon + text mode** — set `brand.icon` when you only have a square mark and want Martis to compose the row as `[your icon] {brand.name}`. Use this when the wordmark is not part of your asset.
- **Default** — leave both null and the bundled Martis cube + text combo renders.
- **Both set** — allowed. `logo` wins on every surface (sidebar, topnav, auth frame). Flip from logo-only mode to icon + text mode by clearing `MARTIS_BRAND_LOGO`, no code change.
