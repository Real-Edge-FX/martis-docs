...
|-----|------|---------|-------------|
| `guard` | `?string` | `null` | Authentication guard. `null` uses Laravel's default guard. The panel's requests run as that guard (it becomes the request's guard), and the auth flows, the Martis migrations and `martis:user` use its provider: see [Upgrading → A custom Martis guard](https://github.com/Real-Edge-FX/martis-package/blob/main/docs/upgrading.md#a-custom-martis-guard). |
