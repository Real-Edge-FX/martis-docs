---
title: "Configuration"
description: "Martis is configured through `config/martis.php`. Publish it with:"
...
| `logo` | `?string` | `null` | **Full horizontal lockup** (icon + wordmark in one asset). When set, the SPA renders the lockup alone — the separate `brand.name` text next to it is hidden in the sidebar / topbar / auth frame to avoid a duplicated wordmark. |
...
| `version` | `?string` | `null` | Optional version string printed in the sidebar footer (e.g. `v1.6.3`, `2026.04.30`). |
...
Renders the lockup image alone. The `brand.name` text is **hidden** next to it. Best when the SVG already contains the mark + wordmark in one piece.
...
Renders `[icon] {brand.name}`. Same shape as the bundled experience but with your icon. Best when the wordmark isn't part of your asset, or when the brand text needs to vary per locale.
...
| Auth card (Login, Register, Forgot password, Reset password, 2FA challenge, error screens) | `brand.logo` → `brand.icon` → bundled lockup |
| Browser tab favicon | `brand.favicon` (independent — see below) |
| Browser tab title | `brand.page_title` + `brand.name` (independent — see below) |
...
The config cache holds the env values; the Martis cache holds the navigation payload. Both must be cleared for the mode swap to land. The frontend bundle does NOT need rebuilding — `window.MartisConfig` is rendered on every request.
...
When only one variant of a pair is set, it is reused for both themes (backwards-compat).
...
The package clamps both values to the listed ranges. The icon-mode size (28×28 menu, 32×32 auth) is fixed.
...
When the sidebar collapses (240 px → 64 px rail) **and** `brand.icon` is set, the icon wins regardless of `brand.logo`. A horizontal lockup at 64 px rail width gets distorted; the square icon fits cleanly. When only `brand.logo` is set, the logo shrinks (the v1.6.x fallback). The brand text is always hidden in collapsed mode.
...
Custom themes are scaffolded via `php artisan martis:theme`. See [Theming](/docs/customization/components#theming).
...
### Customising the greeting strings

The dashboard renders TWO independent welcome strings — make sure you edit the right one:

| Visible string | Translation key | Customisation |
|---|---|---|
| `Hello, {name}` (greeting line) | `martis::resources.hello` | Lang files only |
...
| `<WelcomeCard>` body | `martis::resources.welcome_card_description` | `MARTIS_WELCOME_DESCRIPTION` env **or** lang files |

The `<WelcomeCard>` is a separate animated hero — only visible when `dashboard.showWelcomeCard !== false`. The env vars `MARTIS_WELCOME_HEADING` / `MARTIS_WELCOME_DESCRIPTION` only touch this card; **they do NOT change** the greeting subtitle.

To customise the greeting subtitle:
...
```php
// resources/lang/vendor/martis/{en,pt_BR,pt_PT}/resources.php
return [
    'welcome' => 'Welcome to EdgeFlow — your market quality terminal.',
];
```

Laravel deep-merges, so unset keys fall through.
...
Format: comma-separated `name:hex` pairs.

**Validation rules** (invalid entries dropped silently with a `Log::warning`):
...
| Limit | Up to 24 custom accents. |

Each registered accent:

- Adds an extra swatch at the end of the PreferencesMenu accent picker.
- Becomes a valid value for `MARTIS_DEFAULT_ACCENT`.
- Becomes a valid value for the user's persisted `accent` column.

Hover / soft / strong CSS variants auto-derive from the base hex via `color-mix(in srgb, ...)` (Chrome 111+, Safari 16.2+, Firefox 113+) — the consumer only ships one colour per accent.
