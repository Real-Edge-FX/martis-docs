---
title: "Martis API — Overview"
description: "Martis exposes a REST API for every CRUD operation, resource metadata, navigation, search, preferences, sessions, impersonation, and admin surfaces. The schema is generated at runtime by Scramble and served as both interactive UI and raw OpenAPI 3.1."
sourcePath: "martis-package/docs/api/overview.md"
---
...
Martis is **session-cookie based**. The Laravel guard sets the cookie on `attempt()` + `session()->regenerate()`. There is no JWT; no `Authorization: Bearer …` header is needed (or accepted) by any Martis endpoint. Same-origin SPA calls work out of the box. See [Authentication](/docs/auth/authentication) for the full surface.
...
**Public** (no auth required). Bundled locales: `en`, `pt_BR`, `pt_PT` (underscore form is canonical; the controller normalises hyphenated input). See [i18n](/docs/customization/i18n) for the merge order, `app_namespaces`, and fallback chain.
...
Cross-resource record search. Powers the topbar search input. Each resource is matched on the `searchable()` fields the user can see (v1.38.0), among the records its index lists: its `scopes()`, then `indexQuery()` (v2.0). See [Global Search](/docs/core/global-search).
...
Surface for the [Custom Tools](/docs/customization/tools) primitive.
...
The 404-when-denied behaviour is intentional: an unauthorised user cannot probe which tools the app ships. The routes a tool adds under `/martis/api/tools/{uriKey}/...` (`/{martis.path}/...`) with `loadRoutes()` or `ToolRoutes::middleware()` run the same middleware as these endpoints and answer that user the same 404 (v2.0; see [Tools → Tool routes and their middleware](/docs/customization/tools#tool-routes-and-their-middleware)).
...
PUT body is partial — pass only the keys you want to change. Returns the full snapshot so the React shell can re-bootstrap. See [User Preferences](/docs/reference/preferences).
...
All endpoints scope to the authenticated user. Cross-user access returns 404. See [Notifications](/docs/reference/notifications).
...
See [Cache Control Surface](/docs/reference/cache).
...
See [Authentication § Browser sessions](/docs/auth/authentication#browser-sessions).
...
See [Impersonation](/docs/auth/impersonation).
...
Returns the URL the editor inserts inline. See [Fields § Trix](/docs/core/fields).
...
[Repeater § Validation](/docs/core/repeater#validation)). `code` is a best-effort
