...
description: "Martis ships with a set of React components that make up the admin panel UI. All components are designed to be replaceable through the Override System."
sourcePath: "martis-package/docs/components.md"
---

# Built-in Components Reference
Martis ships with a set of React components that make up the admin panel UI. All components are designed to be replaceable through the [Override System](/docs/customization/overrides).
...
See [Authentication](/docs/auth/authentication) for the full config surface (`config.auth`), backend recipes for SSO / Google, and the registration contract.
...
The layout wraps every page of that resource (index, lens, create, detail, update) inside the shell and receives the page as `children`. See [Overrides → Layout Overrides](/docs/customization/overrides#2-layout-overrides).
...
The `useModalHistoryLock(open)` hook intercepts the browser back button while the dialog is visible and cooperates with the DrawerShell so closing the dialog does not also close the drawer underneath. Required whenever a modal nests inside a drawer or the unsaved-changes guard. It is on `@martis/runtime` since v1.38.0 and has to come from there: it shares a lock count with the drawers, which a copy of the hook would not see. `createPortal` comes from the runtime too: it is the host's, so the dialog renders with the host's React DOM. Since v1.38.0 `import { createPortal } from 'react-dom'` reaches the same function, since the extension build sends `react-dom` to a shim that carries it and `flushSync` (v1.38.2), nothing else of `react-dom`; on a scaffold published earlier, `react-dom` resolves to the React shim, which exports React core only (see [Refreshing the extension scaffold](/docs/getting-started/installation#refreshing-the-extension-scaffold-after-an-upgrade)).
...
Slide-in panels for inline CRUD operations. See [Override System](/docs/customization/overrides) for configuration details.
...
Wraps the create form inside a `DrawerShell`. Sections, Panels and Tabs lay their fields out on the responsive field grid (`colSpan` / `colSpanMd` / `colSpanLg`, see [Grid Layout](/docs/core/grid-layout#responsiveness)); a field outside any layout container is a full-width row.
...
**Configuration:** All visual options (message, icon, logo, colors, opacity) are controlled via `config/martis.php` under the `loader` key. See the [Loader documentation](/docs/customization/loader) for the full configuration reference and customization guide.
...
Every component inside Martis is themed via design tokens (see [theming.md](/docs/customization/theming)), but a small set of standalone utility classes is also exposed so consumer overrides, custom actions and bespoke pages can drop in chips, badges and avatars without reinventing them.
...
| `martis-field-grid` | Responsive field grid: tracks from `--martis-field-columns` (12 by default), each child placed from its `--martis-field-span` / `-md` / `-lg` custom properties, full row below 768px. See [Grid Layout](/docs/core/grid-layout#responsiveness). |
