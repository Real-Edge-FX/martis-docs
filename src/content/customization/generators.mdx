---
title: "Customising Martis Generators"
description: "Every Martis make-command (e.g. martis:resource, martis:action, martis:lens, martis:trend, martis:tool) renders a class from a .stub template shipped inside the package. The defaults aim for \"drop-in\" with sane Laravel-style headers and docblocks. Once a project has its own conve"
sourcePath: "martis-package/docs/customizing-generators.md"
---
...
> **Stub overrides vs component overrides.** This page is about **scaffolding** — the source code emitted once when you run `php artisan martis:resource Foo`. It does not affect runtime rendering. To swap a React component the panel uses live, see the [Override System](/docs/customization/overrides) (`componentRegistry`, `layoutRegistry`).
...
| `agents/AGENTS.md.stub` | `martis:agents`: the guidelines primer for AI coding agents, published to `stubs/martis/agents/` (v1.38.1+; see [Agent guidelines → Customising the primer](/docs/customization/agent-guidelines#customising-the-primer)) |
