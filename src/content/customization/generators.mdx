...
description: "Every Martis make-command (e.g. `martis:resource`, `martis:action`, `martis:lens`, `martis:trend`, `martis:tool`) renders a class from a `.stub` template shipped inside the package. The defaults aim for \"drop-in\" with sane Laravel-style headers and docblocks. Once a project has i"
