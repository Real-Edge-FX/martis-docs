---
title: "Agent Guidelines and the Martis MCP Server"
description: "Generate guidelines for AI coding agents (Claude Code, Codex, Cursor, Gemini, Copilot) and optionally wire the Martis MCP server so the agent can fetch deep dives on demand."
sourcePath: "martis-package/docs/agent-guidelines.md"
---
...
`docs/*.md` files in `vendor/`. The MCP returns scoped, ranked, token-cheap
results and is the single source of truth. If a tool call reports
`enabled: false` (or errors), the guidance is to **stop and ask the operator to
re-enable/restart the MCP**, not to fall back to the files. When no MCP is
wired, the guidelines list the file paths, because then the files are the only
source.

### Optional: machine-enforced doc guard (`--with-doc-guard`)

For a fool-proof guarantee on **Claude Code**, `--with-doc-guard` installs
`.claude/martis-doc-guard.php` plus a `PreToolUse` hook (matcher
`Bash|Read|Grep|Glob`) that blocks any tool call which *reads* a Martis doc from
the filesystem and steers the agent to the MCP. It inspects the specific
tool-input fields, so a command that merely *mentions* the docs directory as a
search string is **not** blocked. Idempotent, and it preserves any hooks you
already have. Pair it with `--with-mcp` so the agent has the MCP to fall back
to. It is Claude-Code specific; other agents use different hook mechanisms.
