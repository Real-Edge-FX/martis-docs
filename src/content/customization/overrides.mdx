...
`DrawerDetail`, `DrawerUpdate` and `DrawerQuick` tell the relationship panels inside them which record they belong to, since the page behind the drawer may not name it (see [relationships.md § Which record a panel belongs to](/docs/core/relationships#which-record-a-panel-belongs-to)).
