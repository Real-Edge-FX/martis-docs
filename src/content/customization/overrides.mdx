...
v1.9.x retired the legacy `@user` build-time alias and the `MARTIS_USER_DIR` symlink mode. The consumer-side build (`npm run build:extensions` against `resources/js/martis-extensions/`) is the only path. Auto-discovery glob picks up every TSX under the four buckets at build time and registers them through `window.Martis.componentRegistry` at runtime.
...
Any key that appears in `martis:list-overrides` but not in `componentRegistry.keys()` is a missing registration in your extension bundle entry — the most common reason an override fails to resolve.
