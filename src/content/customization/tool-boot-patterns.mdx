---
title: "In-app Tools: when (and how) to use `boot()`"
description: "An in-app Tool's setup code can live in the Tool's own boot() or in AppServiceProvider::boot(). This page says when each one runs and shows the four usual pieces (routes, gates, schedules and listeners) wired in a Tool's boot()."
sourcePath: "martis-package/docs/tool-boot-patterns.md"
---

# In-app Tools: when (and how) to use `boot()`
> Companion notes to [tools.md](/docs/customization/tools). Focused on Tools that live INSIDE your application (not Composer packages). For the contrast with Nova 5 conventions (auto-loaded `routes/tool.php`, `Tool::menu()`, sub-nav slot, etc.), see [Differences from Nova 5](/docs/customization/tools#differences-from-nova-5).
>
...
See [Loading routes from a sibling file](/docs/customization/tools#loading-routes-from-a-sibling-file) for the full signature, including custom `middleware` and `prefix`. The inline group above remains valid; `loadRoutes()` is a readability optimisation, not a replacement.
