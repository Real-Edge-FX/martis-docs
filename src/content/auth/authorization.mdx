...
description: "Martis uses the standard Laravel policy system. Every write-side endpoint consults a Laravel policy; the frontend receives the resolved booleans and hides or disables controls accordingly. The backend remains the source of truth — every request is re-authorized server-side even i"
