...
description: "Martis authorizes through the standard Laravel policy system: every write-side endpoint consults a Laravel policy. The frontend receives the resolved booleans and hides or disables controls accordingly."
sourcePath: "martis-package/docs/authorization.md"
---
...
[Resources → routable](/docs/core/resources)) stays usable as a relation target.
...
[Relationships → Panels follow the related resource's `viewAny`](/docs/core/relationships#panels-follow-the-related-resources-viewany-v201).
...
- `Field::canSee(Closure)`: hides a field from every context. Inside a
  ...
  field's `default()`). See
  [Repeater → Readonly, computed, hidden and immutable row fields](/docs/core/repeater#readonly-computed-hidden-and-immutable-row-fields).
  ...
  [Relationships → With Pivot Fields](/docs/core/relationships#with-pivot-fields).
  ...
  [Actions → Fields the request cannot set](/docs/core/actions#fields-the-request-cannot-set).
...
The schema describes the resource, not a record, so a form still lists a field the callback hides for the record it shows: its value is empty, and the save ignores it. See [Fields → Field authorization](/docs/core/fields#field-authorization-cansee-and-canseeformodel).
...
- **The global search** (`/api/search`, the Cmd+K palette): its results and the `total` of each group, on the database and the Scout paths. See [Global Search → Which records are searched](/docs/core/global-search#which-records-are-searched).
- **The records an action runs on** (see [Actions](/docs/core/actions)).
...
Neither hook applies to the relationship pickers (BelongsTo dropdowns, attach pickers): they list through [`relatableQuery()`](/docs/core/resources#relatablequery), as Nova's pickers do ([Nova → Relatable Filtering](https://nova.laravel.com/docs/v5/resources/authorization#relatable-filtering)), so declare the tenant predicate there too. The detail, update and delete endpoints rely on the policies, and so does the parent record of the `HasMany`, `HasOne`, `MorphMany` and `MorphOne` panels and of the `MorphToMany` panel's own endpoints (see [Relationships → How a panel finds its parent record](/docs/core/relationships#how-a-panel-finds-its-parent-record)); a lens owns its query.
