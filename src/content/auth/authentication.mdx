...
> See also: [SSO](/docs/auth/sso) for OAuth/OIDC providers (Azure / Google / GitHub / custom), and [Impersonation](/docs/auth/impersonation) for the v0.10 login-as-another-user subsystem.
...
| Block | Shape | Purpose |
|---|---|---|
| `sso` | `enabled` (master switch) + `providers` (per-provider nested map) | Renders one button per enabled provider. Each button redirects to `/{martis-path}/sso/{provider}/redirect`, which Martis owns end-to-end (no consumer-supplied URL). See [`sso.md`](/docs/auth/sso). |
...
For the full provider schema (role mapping, permission adapters, identity matching) see [`sso.md`](/docs/auth/sso).
