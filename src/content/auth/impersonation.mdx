---
title: "Impersonation"
description: "Support, debugging, and customer-success workflows often need an operator to see exactly what another user sees. Impersonation is the right primitive when you trust the operator and audit-log every session."
sourcePath: "martis-package/docs/impersonation.md"
---
...
Browse them under `/martis/system/action-events` (or whatever URL the bundled `ActionEventResource` lives at), once the `view-martis-action-events` gate lets you (see [Actions → Who can read the audit log](/docs/core/actions#who-can-read-the-audit-log-v201)). Toggle the audit-row write per-environment via `MARTIS_AUDIT_IMPERSONATION=false`: the events still fire so any custom listeners you attach keep firing; only the Martis row is suppressed.
