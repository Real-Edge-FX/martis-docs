---
title: "SSO (Single Sign-On) — ⭐ Martis differential"
description: "Martis signs admin users in through an external identity provider. Azure AD ships built in; Google, GitHub, Okta or any other provider plugs in through MartisSso::extend(). Groups map to your roles (Spatie laravel-permission is detected) and martis:sso scaffolds the setup."
sourcePath: "martis-package/docs/sso.md"
---
...
> Looking to swap the React `LoginPage` (multi-provider button strip + email/password form) for a custom shell? See [Override System](/docs/customization/overrides) — the auth pages sit under registry keys `auth:login`, `auth:register`, `auth:forgot-password`, `auth:reset-password`, `auth:email-verify-notice`.
