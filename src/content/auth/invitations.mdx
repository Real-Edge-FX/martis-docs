---
title: "Invitations"
description: "Not every app wants an open `/register` page. Invite-only onboarding (agencies, internal tools, B2B admin panels, anything with a closed user base) is a different shape: a privileged operator decides who gets in, the invitee never chooses their own email, and the whole thing has"
sourcePath: "martis-package/docs/invitations.md"
---
...
Do **not** reach for this seam for app-wide registration changes — rebind `RegistersUsers` instead (see [authentication.md](/docs/auth/authentication)). Override `createUser()` only for behaviour specific to the invitation flow (reading `metadata`, skipping a step self-registration always does, etc).
...
Two of the same two paths every other auth screen supports (see [authentication.md § Customising the auth copy](/docs/auth/authentication#customising-the-auth-copy)):
