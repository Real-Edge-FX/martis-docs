...
Register the policy once in your service provider:
...
When `martis:sso azure --with-spatie --with-migration` ran in the same project, Spatie roles gain a `provider_group_name` column (see [SSO](/docs/auth/sso#7-spatie--laravel-permission-integration)). The generated `UserResource` filters out any role tied to an SSO provider from its picker, because:
...
Auto-detection for Laravel 11+ is tracked in [issue #122](https://github.com/Real-Edge-FX/martis-package/issues/122) for v1.7.0.
...
Remove the `Gate::policy(...)` block from your service provider and (optionally) the `HasRoles` trait from your User model.
