...
description: "One-shot generator that scaffolds an admin UI for users, roles, and permissions on top of spatie/laravel-permission. Resources land in a unified System sidebar group."
sourcePath: "martis-package/docs/roles.md"
---
...
# Or pick the lowest-id user (typical fresh-install scenario):
php artisan martis:roles --promote=first
```

Without the flag, the manual fallback in `tinker` still works:
...
When `martis:sso azure --with-spatie --with-migration` ran in the same project, Spatie roles gain a `provider_group_name` column (see [`docs/sso.md`](/docs/auth/sso#7-spatie--laravel-permission-integration)). The generated `UserResource` filters out any role tied to an SSO provider from its picker, because:
...
`Martis\Http\Controllers\NavigationController` builds the sidebar by iterating the `ResourceRegistry`. Resources whose `belongsToSystemSection()` returns `true`, and Tools that call `withSystemSection()` (v1.35.0+), are pulled out of the regular grouping loop and merged with the Cache admin link into a single **System** menu section (resources first, then tools, then the Cache admin link, unless weights reorder it: `systemSectionOrder()` on resources and tools, `martis.cache.admin_ui_order` for the link, since v1.38.0; see [Menus → Order inside the System section](/docs/core/menus#order-inside-the-system-section-v1380)). The section appears whenever there is at least one item visible to the current user — no items, no section.
...
- [Authorization](/docs/auth/authorization) — policy patterns and gate basics.
- [SSO](/docs/auth/sso) — wire Azure AD, Google, or a custom IdP and let it manage a subset of roles.
- [Resources](/docs/core/resources) — extend the generated User / Role / Permission classes with fields, filters, lenses.
