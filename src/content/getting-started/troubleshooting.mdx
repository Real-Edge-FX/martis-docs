...
- **`boot.ts` not loaded.** The generated `boot.ts` is auto-imported only when the consumer's `resources/martis-extensions/` folder is found at build time. See [Override System](/docs/customization/overrides) for the resolution chain and the `MARTIS_USER_DIR` opt-in for monorepo setups.
