...
export function CmdK({ open, onClose }: CmdKProps) {
  ...
  // Reset the palette's local state the instant it opens. Adjusting
  // state during render (rather than in an effect) applies it before
  // this render paints, instead of one tick later.
  ...
  // The setTimeout gives the modal a tick to mount before we steal
  // focus — without it Safari sometimes loses the keyboard event that
  // opened us.
  useEffect(() => {
    if (!open) return
  ...
  // Skipped for a blank query — `flat` below already ignores
  // `fullTextHits` in that case, so there is nothing to reset here.
  useEffect(() => {
  ...
  const flat = useMemo(() => {
    ...
    const staticHits = trimmed ? searchStatic(q) : STATIC_INDEX
    const fullText = trimmed ? fullTextHits : []
    if (!fullText.length) return staticHits
    ...
    const extras = fullText.filter((h) => !seen.has(h.href))
  ...
  return (
    <div className="fixed inset-0 z-[100] fade-in flex items-start justify-center pt-[12vh] px-4">
      <button
        type="button"
      ...
      <div
        ...
        className="relative z-[1] cmdk-anim w-full max-w-[640px] bg-ink-850/95 backdrop-blur-xl ring-1 ring-white/10 rounded-2xl shadow-2xl overflow-hidden"
      >
